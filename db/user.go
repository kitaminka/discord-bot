...
func ResetUserMuteCount(userID string) error {
	_, err := MongoDatabase.Collection(UserCollectionName).UpdateOne(context.Background(), bson.D{{"id", userID}}, bson.D{{"$unset", bson.D{{"lastMuteTime", nil}, {"muteCount", nil}}}}, options.Update().SetUpsert(true))
	return err
}
